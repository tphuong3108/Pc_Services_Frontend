  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h1 className="text-xl font-semibold mb-6">
        Kết quả tìm kiếm: <span className="text-blue-600 font-medium">"{query}"</span>
      </h1>

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map((item) => {
          const hasDiscount = (item.sale_off ?? 0) > 0;
          const finalPrice = item.price - (hasDiscount ? (item.price * (item.sale_off ?? 0)) / 100 : 0);
          const isOutOfStock = item.quantity === 0;

          return (
            <div key={item._id} className="border border-gray-200 rounded-lg p-3 hover:shadow-md transition relative">
              <Link href={`/user/product/detail/${item.slug}`}>
                ...
                <div className="flex items-center justify-between mt-2">
                  <div>
                    {isOutOfStock ? (
                      <span className="text-gray-500 font-medium text-sm">Tạm hết hàng</span>
                    ) : (
                      <>
                        <span className="text-red-500 font-semibold text-sm">{finalPrice.toLocaleString()}₫</span>
                        {hasDiscount && (
                  ...
                  <div className="flex items-center text-xs text-gray-500">
                    ...
                    {typeof item.rating === "number" ? (item.rating || 5).toFixed(1) : "5.0"}
                  </div>
                </div>
              </Link>
            </div>
        ...
        {products.length === 0 && (
          <p className="text-gray-500 text-sm col-span-full">Không tìm thấy sản phẩm phù hợp với từ khóa.</p>
        )}
      </div>
